...
## Cross-Vendor 3DGS Engine Status

This repository is a public utility spin-off from a proprietary, CUDA-bypassing 3D Gaussian Splatting commercial engine. Current hardware backend status:

* **AMD (RDNA4 / RX 9070 XT):** 🔴 **Blocked.** Currently halted by a critical L2 TLB driver fault within the ROCm `rocBLAS` GEMM kernel. See the [public diagnostic report](reports/AMD_gfx1201_UTCL2_TLB_FAULT/) for full hardware-level forensics.
* **Intel (Xe2 / Arc B580):** 🟢 **Fully Stable.** The entire pipeline runs to completion via custom SYCL polyfills and native OpenVINO integrations. 
* **Performance & Stability Note:** In PyTorch inference (LightGlue), the AMD 9070 XT holds a ~15% performance lead over the Intel Arc B580, but ultimately cannot complete the workload due to the fatal L2 TLB coherency fault. However, when executing the exact same LightGlue workload natively via ONNX Runtime, the entry-level Intel Arc B580 not only runs with 100% stability, but actually outperforms the AMD 9070 XT by a factor of **2.2x** (despite a 3.5x raw FP32 compute disadvantage).

> **Note:** The custom Intel Xe SYCL polyfills, Battlemage backend workarounds, and OpenVINO engine source code are currently held in a **private repository** pending commercial launch and hardware partner review.
...
| Report | Description |
|--------|-------------|
| **[AMD gfx1201 UTCL2 TLB Fault](reports/AMD_gfx1201_UTCL2_TLB_FAULT/)** | 34 forensically documented crashes in rocBLAS GEMM kernel on RDNA4 — cross-validated across PyTorch and ONNX Runtime |
...
```bash
git clone [https://github.com/YOUR_USERNAME/linux-gpu-tools.git](https://github.com/YOUR_USERNAME/linux-gpu-tools.git)
cd linux-gpu-tools
...
Background
​These tools were born out of necessity while debugging a persistent UTCL2 TLB coherency fault on AMD's RDNA4 architecture (gfx1201). The fault occurs in the rocBLAS GEMM kernel (label_LoopBeginL / label_LoopEndL) during sustained FP16 matrix multiplication workloads and has been reproduced 34 times across:
...
​See the full forensic report for complete technical details, fault register analysis, and GPU wave state captures.
...
License
​Apache 2.0 — Use freely in personal, open-source, or commercial projects.
​Contributing
